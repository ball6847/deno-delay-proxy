...
import type { KillSwitchData } from "./schemas.ts";
...
export class KillSwitchRepository {
	...
	async load(): Promise<Result<KillSwitchData, Error>> {
		const entryResult = await Result.try(() =>
			this.kv.get<KillSwitchData>(KILL_SWITCH_KEY)
		);
	...
	async save(data: KillSwitchData): Promise<Result<void, Error>> {
